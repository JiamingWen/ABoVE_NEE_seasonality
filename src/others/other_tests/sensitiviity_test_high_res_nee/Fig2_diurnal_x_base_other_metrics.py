...
for the case of imposing X-BASE diurnal cycle
'''
...
# stat_var = 'cor'; xlim = [-0.2, 0.85]; xlabel = r'Correlation with CO$_{2}$ observations'; subtitle = '(a)'
# stat_var = 'slope'; xlim = [-0.2, 2.2]; xlabel = r'Slope of regression with CO$_{2}$ observations'; subtitle = '(d)'
stat_var = 'intercept'; xlim = [-10, 2]; xlabel = r'Intercept of regression with CO$_{2}$ observations'; subtitle = '(e)'
# stat_var = 'mean_bias'; xlim = [-10, 2]; xlabel = r'Mean bias compared to CO$_{2}$ observations (ppm)'; subtitle = '(b)'
# stat_var = 'rmse'; xlim = [0, 15]; xlabel = r'RMSE compared to CO$_{2}$ observations (ppm)'; subtitle = '(c)'
...
fitting_df_TRENDYv11_sorted = fitting_df_TRENDYv11_unscaled.sort_values('cor')
fitting_df_inversions_sorted = fitting_df_inversions_unscaled.sort_values('cor')
fitting_df_UpscaledEC_sorted = fitting_df_UpscaledEC_unscaled.sort_values('cor')
...
if stat_var == 'cor':
    plt.legend(bbox_to_anchor=(0, 0.9), fontsize=14, loc='upper left')
ax.text(0.9, 0.05, subtitle, transform=ax.transAxes, fontsize=18, verticalalignment='top', horizontalalignment='left')

fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_test_high_res_nee/Fig2_diurnal_x_base_other_metrics_{stat_var}.png', dpi=300, bbox_inches='tight')
fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_test_high_res_nee/Fig2_diurnal_x_base_other_metrics_{stat_var}.pdf', dpi=300, bbox_inches='tight')
plt.show()
'''convert daily fluxes to the concentration space'''
...
for year in [2012, 2013, 2014, 2017]:
    ...
    # read H matrix
    h_matrix = read_H_matrix_daily(year, n_receptor, land_cellnum_list)
    ...
    for nday in np.arange(0, config["ndays"]):
        ...
        NEE_vec = subset_30N_90N(read_CT_NOAA_daily(timestep.year, timestep.month, timestep.day)).values.flatten()[land_cellnum_list] #unit: mol m-2 s-1
    ...
    for nday in np.arange(0, config["ndays"]):
        ...
        timestep = config["sdate"] + nday * datetime.timedelta(days=1)
        NEE_vec = subset_30N_90N(read_CTE_daily(timestep.year, timestep.month, timestep.day)).values.flatten()[land_cellnum_list] #unit: mol m-2 s-1
        NEE = NEE_vec * 1e6 #convert unit to μmol m-2 s-1
    ...
    for nday in np.arange(0, config["ndays"]):
        ...
        NEE_vec = subset_30N_90N(read_x_base_daily(timestep.year, timestep.month, timestep.day)).fillna(0).values.flatten()[land_cellnum_list] # unit gC m-2 d-1

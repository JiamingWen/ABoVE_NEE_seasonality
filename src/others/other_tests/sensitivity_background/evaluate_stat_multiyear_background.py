...
use all years' data
use Carbon Tracker background values to calculate CO2 enhancement
'''
...
for model_type in model_types:
    ...
    for (model_id, model_name) in enumerate(model_names):
        ...
        for year in [2012, 2013, 2014, 2017]: #2012, 2013, 2014, 2017
            ...
            df_airborne = pd.read_csv(f'/central/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/data/{campaign_name}_airborne/atm_obs/ABoVE_{year}_{campaign_name}_airborne_change_background-{background}.csv')
        ...
        if model_type in model_types_single:
            ...
            fitting_df_unscaled.to_csv(f"{dir0}evaluation_stat_unscaled_{model_type}{lc_filestr}_background-{background}.csv", encoding='utf-8', index=False)
    ...
    if model_type in model_types_single:
        ...
        plt.savefig(f"{dir0}evaluation_stat_{model_type}{lc_filestr}_background-{background}_scatterplot.png", dpi=100, bbox_inches='tight')
        
        
    fitting_df.to_csv(f"{dir0}evaluation_stat_scaled_{model_type}{lc_filestr}_background-{background}.csv", encoding='utf-8', index=False)

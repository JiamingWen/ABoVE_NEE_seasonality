...
adopted from evaluate_stat_multiyear_highres.py, but use only afternoon observations
'''
...
dir0 = f"/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/"
dir1 = f"/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/"
...
for model_type in model_types:
    ...
    for (model_id, model_name) in enumerate(model_names):
        ...
        for year in [2012, 2013, 2014, 2017]: #2012, 2013, 2014, 2017
            ...
            mask_id = np.where((df_airborne['background_CO2_std'].notna()) &
                (df_airborne['local_hour'].isin([14, 15, 16, 17])) & # 14:00-18:00
        ...
        fitting_df_unscaled.to_csv(f"{dir0}evaluation_stat_unscaled_{model_type}{lc_filestr}_highres_afternoon.csv", encoding='utf-8', index=False)
        ...
        model = sm.OLS(y,X)
        results2 = model.fit()
        results2.save(f"{dir1}{model_type}_{model_name}{lc_filestr}_highres_afternoon.pickle")
        # to load ols model back
        results2 = OLSResults.load(f"{dir1}{model_type}_{model_name}{lc_filestr}_highres_afternoon.pickle")
        results2.params
        results2.summary() # this is exactly the same as the results from fit_linear_regression_fullH.py
        # # export results to txt
        # f = f"{dir1}{model_type}_{model_name}{lc_filestr}_highres_afternoon.txt"
        # with open(f, 'w') as fh:
    ...
    plt.savefig(f"{dir0}evaluation_stat_{model_type}{lc_filestr}_highres_afternoon_scatterplot.png", dpi=100, bbox_inches='tight')
    plt.show()
        
    fitting_df.to_csv(f"{dir0}evaluation_stat_scaled_{model_type}{lc_filestr}_highres_afternoon.csv", encoding='utf-8', index=False)

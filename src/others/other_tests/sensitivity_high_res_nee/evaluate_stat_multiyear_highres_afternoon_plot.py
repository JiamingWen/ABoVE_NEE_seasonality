'''plot evaluation statistics of model outputs at various temporal resolutions using afternoon only data'''
...
# X-BASE
x_base_df = pd.read_csv('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/evaluation_stat_unscaled_X-BASE_highres_afternoon.csv')
if replace_monthly:
	x_base_df.loc[x_base_df['model_name'] == 'X-BASE-monthly', x_base_df.columns[1:]] = upscaledEC_df[upscaledEC_df['model_name'] == 'X-BASE'].iloc[:, 1:].values
else:
	x_base_df = pd.concat([x_base_df, upscaledEC_df[upscaledEC_df['model_name'] == 'X-BASE']], ignore_index=True)
...
# CTE
cte_df = pd.read_csv('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/evaluation_stat_unscaled_CTE_highres_afternoon.csv')
if replace_monthly:
	cte_df.loc[cte_df['model_name'] == 'CTE-monthly', cte_df.columns[1:]] = inversion_df[inversion_df['model_name'] == 'CTE'].iloc[:, 1:].values
else:
	cte_df = pd.concat([cte_df, inversion_df[inversion_df['model_name'] == 'CTE']], ignore_index=True)
...
# CT-NOAA
ct_noaa_df = pd.read_csv('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/evaluation_stat_unscaled_CT-NOAA_highres_afternoon.csv')
if replace_monthly:
	ct_noaa_df.loc[ct_noaa_df['model_name'] == 'CT-NOAA-monthly', ct_noaa_df.columns[1:]] = inversion_df[inversion_df['model_name'] == 'CT-NOAA'].iloc[:, 1:].values
else:
	ct_noaa_df = pd.concat([ct_noaa_df, inversion_df[inversion_df['model_name'] == 'CT-NOAA']], ignore_index=True)
...
fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/evaluation_stat_high_res_nee_{stat_var}_afternoon.png', dpi=300, bbox_inches='tight')
fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/afternoon/evaluation_stat_high_res_nee_{stat_var}_afternoon.pdf', dpi=300, bbox_inches='tight')
plt.show()

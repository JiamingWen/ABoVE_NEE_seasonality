'''
This script generates Fig. S6 in SI (stat_var = 'cor').
It compares evaluation statistics of model outputs at various temporal resolutions
'''
...
stat_var = 'cor'; xlim = [-0.1, 0.75]; xlabel = r'Correlation with CO$_{2}$ observations'; label = '(a)'
# stat_var = 'slope'; xlim = [-0.2, 2.2]; xlabel = r'Slope of regression with CO$_{2}$ observations'; label = '(b)'
# stat_var = 'intercept'; xlim = [-10, 2]; xlabel = r'Intercept of regression with CO$_{2}$ observations'
# stat_var = 'mean_bias'; xlim = [-10, 2]; xlabel = r'Mean bias compared to CO$_{2}$ observations (ppm)'; label = '(b)'
...
if replace_monthly:
	x_base_df.loc[x_base_df['model_name'] == 'X-BASE-monthly', x_base_df.columns[1:]] = upscaledEC_df[upscaledEC_df['model_name'] == 'X-BASE'].iloc[:, 1:8].values
else:
...
if replace_monthly:
	cte_df.loc[cte_df['model_name'] == 'CTE-monthly', cte_df.columns[1:]] = inversion_df[inversion_df['model_name'] == 'CTE'].iloc[:, 1:8].values
else:
...
if replace_monthly:
	ct_noaa_df.loc[ct_noaa_df['model_name'] == 'CT-NOAA-monthly', ct_noaa_df.columns[1:]] = inversion_df[inversion_df['model_name'] == 'CT-NOAA'].iloc[:, 1:8].values
else:
...
# X-BASE
plt.scatter(x_base_df[f'{stat_var}'].iloc[-1], x_base_df['model_name'].iloc[-1], marker='d', color='#56983f', facecolor='none', s=70)
for i, marker in enumerate(markers):
	plt.scatter(x_base_df[f'{stat_var}'][len(markers)-1-i], x_base_df['model_name'][len(markers)-1-i], marker=markers[i], color='#56983f', s=size[i])
...
fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/evaluation_stat_high_res_nee_{stat_var}.png', dpi=300, bbox_inches='tight')
fig.savefig(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_high_res_nee/evaluation_stat_high_res_nee_{stat_var}.pdf', dpi=300, bbox_inches='tight')
plt.show()

...
# scaled variables (with linear regression)
fitting_df_reference_scaled_ocean_criterion = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/evaluation_stat/evaluation_stat_scaled_reference{lc_filestr}_ocean_criterion.csv')
fitting_df_reference_scaled_ocean_criterion = fitting_df_reference_scaled_ocean_criterion.loc[fitting_df_reference_scaled_ocean_criterion['model_name'].isin(['APAR', 'GOME2_SIF']), :]  #'APAR', 'FPAR', 'LAI', 'PAR'
fitting_df_reference_scaled_ocean_criterion.loc[fitting_df_reference_scaled_ocean_criterion['model_name'] == 'GOME2_SIF','model_name'] = 'SIF'
...
fitting_df_UpscaledEC_merge = pd.merge(fitting_df_UpscaledEC_sorted, fitting_df_UpscaledEC_unscaled_ocean_criterion, on='model_name', how='outer', suffixes=('_all_criterion', '_ocean_criterion'))
fitting_df_reference_merge = pd.merge(fitting_df_reference_scaled, fitting_df_reference_scaled_ocean_criterion, on='model_name', how='outer', suffixes=('_all_criterion', '_ocean_criterion'))


color = 'red' # color for afternoon data
labelname = f"Removing ocean criterion\n(n = 85693)"

plt.scatter(fitting_df_TRENDYv11_merge[f'{stat_var}_ocean_criterion'], fitting_df_TRENDYv11_merge['model_name'], marker='o', color=color, facecolors='none', label=labelname, s=70)
plt.scatter(fitting_df_inversions_merge[f'{stat_var}_ocean_criterion'], fitting_df_inversions_merge['model_name'], marker='o', color=color, facecolors='none', s=70)
plt.scatter(fitting_df_UpscaledEC_merge[f'{stat_var}_ocean_criterion'], fitting_df_UpscaledEC_merge['model_name'], marker='o', color=color, facecolors='none', s=70)
plt.scatter(fitting_df_reference_merge[f'{stat_var}_ocean_criterion'], fitting_df_reference_merge['model_name'], marker='o', color=color, facecolors='none', s=70)

plt.legend(bbox_to_anchor=(0.65, 0.7), fontsize=14)

fig.savefig('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_ocean_fluxes/Fig2_ocean_criterion.png', dpi=300, bbox_inches='tight')
fig.savefig('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_ocean_fluxes/Fig2_ocean_criterion.pdf', dpi=300, bbox_inches='tight')
plt.show()
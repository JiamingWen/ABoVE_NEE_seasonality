...
for model_type in model_types:
    ...
    for (model_id, model_name) in enumerate(model_names):
        ...
        if model_type in model_types_single:
            ...
            fitting_df_unscaled.to_csv(f"{dir0}evaluation_stat_unscaled_{model_type}{lc_filestr}_ocean_criterion.csv", encoding='utf-8', index=False)
        ...
        model = sm.OLS(y,X)
        results2 = model.fit()
        # results2.save(f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_criterion.pickle")
        # # to load ols model back
        # results2 = OLSResults.load(f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_criterion.pickle")
        # results2.params
        # results2.summary() # this is exactly the same as the results from fit_linear_regression_fullH.py
        # # export results to txt
        # f = f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_criterion.txt"
        # with open(f, 'w') as fh:
    ...
    if model_type in model_types_single:
        ...
        plt.savefig(f"{dir0}evaluation_stat_{model_type}{lc_filestr}_ocean_criterion_scatterplot.png", dpi=100, bbox_inches='tight')
    ...
    fitting_df.to_csv(f"{dir0}evaluation_stat_scaled_{model_type}{lc_filestr}_ocean_criterion.csv", encoding='utf-8', index=False)

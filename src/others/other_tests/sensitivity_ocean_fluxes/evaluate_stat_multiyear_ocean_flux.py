...
remove the data filtering criterion of regional relavance (ABoVE influence > 50%, ocean influence < 30%)
also account for CO2 enhancements due to ocean fluxes from data products' ensemble
'''
...
for model_type in model_types:
    ...
    for (model_id, model_name) in enumerate(model_names):
        ...
        for year in [2012, 2013, 2014, 2017]: #2012, 2013, 2014, 2017
            ...
            # influence from ocean fluxes
            df_ocean = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/data/{campaign_name}_airborne/transported_surface_field/ABoVE_{year}_{campaign_name}_airborne_ocean.csv')

            # derive biogenic CO2 drawdown/enhancement from fossil and fire emissions
            y0 = df_airborne['CO2_change'] - df_fossil['odiac2022'] - df_fire['gfed4.1'] - df_ocean.mean(axis=1)
        ...
        if model_type in model_types_single:
            ...
            fitting_df_unscaled.to_csv(f"{dir0}evaluation_stat_unscaled_{model_type}{lc_filestr}_ocean_flux.csv", encoding='utf-8', index=False)
        ...
        model = sm.OLS(y,X)
        results2 = model.fit()
        # results2.save(f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_flux.pickle")
        # # to load ols model back
        # results2 = OLSResults.load(f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_flux.pickle")
        # results2.params
        # results2.summary() # this is exactly the same as the results from fit_linear_regression_fullH.py
        # # export results to txt
        # f = f"{dir1}{model_type}_{model_name}{lc_filestr}_ocean_flux.txt"
        # with open(f, 'w') as fh:
    ...
    if model_type in model_types_single:
        ...
        plt.savefig(f"{dir0}evaluation_stat_{model_type}{lc_filestr}_ocean_flux_scatterplot.png", dpi=100, bbox_inches='tight')
    ...
    fitting_df.to_csv(f"{dir0}evaluation_stat_scaled_{model_type}{lc_filestr}_ocean_flux.csv", encoding='utf-8', index=False)

'''
sensitivity test - replace GFED4.1 with GFED5 for fire emissions
'''
...
for model_type in model_types:
    ...
    for (model_id, model_name) in enumerate(model_names):
        ...
        for year in [2012, 2013, 2014, 2017]: #2012, 2013, 2014, 2017
            ...
            y0 = df_airborne['CO2_change'] - df_fossil['odiac2022'] - df_fire['gfed5']
            ...
            if model_type in model_types_single:
                ...
                if model_type in ['inversionsNEE', 'inversionsNEE-prior']: # account for fire emissions
                    ...
                    X_year_NEE = df_model[f"{model_name}"].loc[mask_id] - df_fire['gfed5'].loc[mask_id]
        ...
        if model_type in model_types_single:
            ...
            fitting_df_unscaled.to_csv(f"{dir0}evaluation_stat_unscaled_{model_type}{lc_filestr}_gfed5.csv", encoding='utf-8', index=False)
        ...
        model = sm.OLS(y,X)
        results2 = model.fit()
        # results2.save(f"{dir1}{model_type}_{model_name}{lc_filestr}_gfed5.pickle")
        # # to load ols model back
        # results2 = OLSResults.load(f"{dir1}{model_type}_{model_name}{lc_filestr}_gfed5.pickle")
        # results2.params
        # results2.summary() # this is exactly the same as the results from fit_linear_regression_fullH.py
        # # export results to txt
        # f = f"{dir1}{model_type}_{model_name}{lc_filestr}_gfed5.txt"
        # with open(f, 'w') as fh:
    ...
    if model_type in model_types_single:
        ...
        plt.savefig(f"{dir0}evaluation_stat_{model_type}{lc_filestr}_gfed5_scatterplot.png", dpi=100, bbox_inches='tight')
        
        plt.show()
        
    fitting_df.to_csv(f"{dir0}evaluation_stat_scaled_{model_type}{lc_filestr}_gfed5.csv", encoding='utf-8', index=False)

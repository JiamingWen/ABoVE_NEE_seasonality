...
evaluation with GridFED fossil fuel emissions
'''
...
##############################################################
# overlay with results for GridFED
if lcname == 'alllc':
    lc_filestr = ''
elif lcname in ['forest', 'shrub', 'tundra']:
    lc_filestr = '_' + lcname
...
# unscaled variables (without linear regression)
fitting_df_TRENDYv11_unscaled_gridfed = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/evaluation_stat/evaluation_stat_unscaled_TRENDYv11{lc_filestr}_gridfed.csv')
# fitting_df_TRENDYv11_unscaled_gridfed = fitting_df_TRENDYv11_unscaled_gridfed.loc[~fitting_df_TRENDYv11_unscaled_gridfed['model_name'].isin(['IBIS']), :] # remove IBIS because it simulates negative Rh
fitting_df_inversions_unscaled_gridfed = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/evaluation_stat/evaluation_stat_unscaled_inversionsNEE{lc_filestr}_gridfed.csv')
fitting_df_inversions_unscaled_gridfed = fitting_df_inversions_unscaled_gridfed.loc[~fitting_df_inversions_unscaled_gridfed['model_name'].isin(['CAMS-Satellite', 'COLA', 'GCASv2', 'GONGGA', 'THU']), :] ## for models with no coverage of CARVE years
fitting_df_inversions_unscaled_gridfed.loc[fitting_df_inversions_unscaled_gridfed['model_name'] == 'MIROC','model_name'] = 'MIROC4-ACTM'
fitting_df_UpscaledEC_unscaled_gridfed = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/evaluation_stat/evaluation_stat_unscaled_UpscaledEC{lc_filestr}_gridfed.csv')

# scaled variables (with linear regression)
fitting_df_reference_scaled_gridfed = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/evaluation_stat/evaluation_stat_scaled_reference{lc_filestr}_gridfed.csv')
fitting_df_reference_scaled_gridfed = fitting_df_reference_scaled_gridfed.loc[fitting_df_reference_scaled_gridfed['model_name'].isin(['APAR', 'GOME2_SIF']), :]  #'APAR', 'FPAR', 'LAI', 'PAR'
fitting_df_reference_scaled_gridfed.loc[fitting_df_reference_scaled_gridfed['model_name'] == 'GOME2_SIF','model_name'] = 'SIF'


fitting_df_TRENDYv11_merge = pd.merge(fitting_df_TRENDYv11_sorted, fitting_df_TRENDYv11_unscaled_gridfed, on='model_name', how='outer', suffixes=('_odiac', '_gridfed'))
fitting_df_inversions_merge = pd.merge(fitting_df_inversions_sorted, fitting_df_inversions_unscaled_gridfed, on='model_name', how='outer', suffixes=('_odiac', '_gridfed'))
fitting_df_UpscaledEC_merge = pd.merge(fitting_df_UpscaledEC_sorted, fitting_df_UpscaledEC_unscaled_gridfed, on='model_name', how='outer', suffixes=('_odiac', '_gridfed'))
fitting_df_reference_merge = pd.merge(fitting_df_reference_scaled, fitting_df_reference_scaled_gridfed, on='model_name', how='outer', suffixes=('_odiac', '_gridfed'))


color = 'red'
labelname = "GridFED"

plt.scatter(fitting_df_TRENDYv11_merge[f'{stat_var}_gridfed'], fitting_df_TRENDYv11_merge['model_name'], marker='o', color=color, facecolors='none', label=labelname, s=70)
plt.scatter(fitting_df_inversions_merge[f'{stat_var}_gridfed'], fitting_df_inversions_merge['model_name'], marker='o', color=color, facecolors='none', s=70)
plt.scatter(fitting_df_UpscaledEC_merge[f'{stat_var}_gridfed'], fitting_df_UpscaledEC_merge['model_name'], marker='o', color=color, facecolors='none', s=70)
plt.scatter(fitting_df_reference_merge[f'{stat_var}_gridfed'], fitting_df_reference_merge['model_name'], marker='o', color=color, facecolors='none', s=70)

plt.legend(bbox_to_anchor=(0.1, 0.7), fontsize=14)

fig.savefig('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_test_fire/Fig2_gridfed.png', dpi=300, bbox_inches='tight')
fig.savefig('/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_test_fire/Fig2_gridfed.pdf', dpi=300, bbox_inches='tight')
plt.show()
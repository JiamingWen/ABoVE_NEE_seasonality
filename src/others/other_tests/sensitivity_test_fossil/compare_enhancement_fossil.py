'''compare CO2 enhancement of fossil fuel emissions from different datasets'''
...
for year in [2012, 2013, 2014, 2017]: #2012, 2013, 2014, 2017
    ...
    if year == 2012:
        ...
        df_fossil = pd.concat((df_fossil, df_fossil_year))

y = df_fossil['gridfed2024']
x = df_fossil['odiac2022']
...
ax.set_xlabel('CO$_2$ enhancement from ODIAC (ppm)', fontsize=12)
ax.set_ylabel('CO$_2$ enhancement from GridFED (ppm)', fontsize=12)
plt.savefig(f"/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/result/other/sensitivity_test_fossil/compare_fossil_scatterplot.png", dpi=100, bbox_inches='tight')
plt.show()
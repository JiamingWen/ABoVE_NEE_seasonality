...
plt.scatter(fitting_df_reference_sorted[f'{stat_var}'], fitting_df_reference_sorted['model_name'], marker='o', color='purple', s=50)
...
plt.scatter(fitting_df_inversions_sorted[f'{stat_var}'], fitting_df_inversions_sorted['model_name'], marker='o', color='black', s=50, label='Original data')

plt.axhline(y = fitting_df_reference_sorted.shape[0]-0.5, color = 'grey', linestyle = '--')
plt.axhline(y = fitting_df_reference_sorted.shape[0] + fitting_df_TRENDYv11_sorted.shape[0]-0.5, color = 'grey', linestyle = '--')
plt.axhline(y = fitting_df_reference_sorted.shape[0] + fitting_df_TRENDYv11_sorted.shape[0]+fitting_df_inversions_sorted.shape[0]-0.5, color = 'grey', linestyle = '--')
plt.xlim(xlim)
plt.ylim(-0.5, fitting_df_TRENDYv11_sorted.shape[0]+fitting_df_inversions_sorted.shape[0]+fitting_df_reference_sorted.shape[0]-0.5)
...
colors = ['purple']*fitting_df_reference_sorted.shape[0] + fitting_df_TRENDYv11_sorted['color'].values.tolist() + ['black']*fitting_df_inversions_sorted.shape[0]
for ytick, color in zip(ax.get_yticklabels(), colors):
    ytick.set_color(color)

plt.annotate("Global Inversions", (-0.05, fitting_df_reference_sorted.shape[0]+fitting_df_TRENDYv11_sorted.shape[0]+fitting_df_inversions_sorted.shape[0]-1.5), fontsize=15)
plt.annotate("TRENDY TBMs", (-0.05, fitting_df_reference_sorted.shape[0]+fitting_df_TRENDYv11_sorted.shape[0]-1.5), fontsize=15)
# plt.annotate("Remote Sensing", (-0.05, fitting_df_reference_sorted.shape[0]-1.2), fontsize=15)

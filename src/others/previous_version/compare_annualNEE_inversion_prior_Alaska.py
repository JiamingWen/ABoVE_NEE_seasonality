...
for year in [2012, 2013, 2014, 2017]:
    ...
    for (model_id, model_name) in enumerate(model_names):
        seasonal_df_prior = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/seasonal/seasonal_{year}_inversionsNEE-prior_{regionname}.csv')
        seasonal_df_posterior = pd.read_csv(f'/resnick/groups/carnegie_poc/jwen2/ABoVE/ABoVE_NEE_seasonality/seasonal//seasonal_{year}_inversionsNEE_{regionname}.csv')
